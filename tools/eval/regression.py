...
def check_ci_thresholds(report: EvaluationReport) -> tuple[bool, list[str]]:
    ...
    if "top_1_below_critical" in ci_config["fail_on"]:
        if report.top_1_path_accuracy < path_thresh["top_1"]["critical"]:
            failures.append(
                f"Top-1 path accuracy ({report.top_1_path_accuracy:.1%}) below critical threshold ({path_thresh['top_1']['critical']:.0%})"
            )

    # Check Top-5 accuracy
    if "top_5_below_critical" in ci_config["fail_on"]:
        if report.top_5_path_accuracy < path_thresh["top_5"]["critical"]:
            failures.append(
                f"Top-5 path accuracy ({report.top_5_path_accuracy:.1%}) below critical threshold ({path_thresh['top_5']['critical']:.0%})"
            )

    # Check latency
    if "latency_above_critical" in ci_config["fail_on"]:
        if report.latency_p95_ms > latency_thresh["p95"]["critical"]:
            failures.append(
                f"Latency p95 ({report.latency_p95_ms:.1f}ms) above critical threshold ({latency_thresh['p95']['critical']}ms)"
            )

    # Check regression
    if "regression_detected" in ci_config["fail_on"]:
        if report.regression and report.regression.has_regression:
            failures.append(
                f"Regression detected: Top-1 delta {report.regression.top_1_delta:+.1%}, "
