...
KB_PATH=tests/fixtures/eval_kb EVAL_GOLDEN_QUERIES=tests/fixtures/eval_golden_queries.yaml TOKENIZERS_PARALLELISM=false uv run python -m tools.eval.cli --full --search adaptive --output-dir docs/benchmarks/performance_loop_single_baseline
```
...
KB_PATH=tests/fixtures/eval_kb EVAL_GOLDEN_QUERIES=tests/fixtures/eval_golden_queries.yaml TOKENIZERS_PARALLELISM=false uv run python -m tools.eval.cli --full --search adaptive --output-dir docs/benchmarks/performance_loop_single_after
```
...
`adaptive` p95 improved by 62.3% and p50 improved by 97.0% on the same local
harness. Treat these as relative fixture numbers, not corpus-scale claims.

## Single-Fixture Adaptive Results
...
- `adaptive` search now trusts a conceptual BM25 result when the top score is
...
rejected because it reduced multi-KB `adaptive` Hit@1 to 97.1%.
